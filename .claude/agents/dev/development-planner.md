...
> **⚠️ 테스트 필수 원칙**: API 연동, 비즈니스 로직, 인증/권한, 데이터 처리 등 모든 핵심 기능 구현 직후 반드시 Playwright MCP로 테스트를 수행해야 합니다. 테스트 통과 확인 전에는 다음 단계로 진행하지 않습니다.

**단계별 구현-테스트 사이클:**

1. 기능 구현 (코드 작성)
2. 개발 서버 실행 확인 (`npm run dev`)
3. Playwright MCP로 해당 기능 테스트 실행
4. 테스트 실패 시 → 원인 파악 후 수정 → 2단계 반복
5. 테스트 통과 확인 후 → 다음 단계 진행
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **구현 직후 반드시 Playwright MCP로 테스트 수행** (API 연동, 비즈니스 로직, 인증 등 모든 핵심 기능)
...
Step 1. 코드 구현
Step 2. 개발 서버 정상 실행 확인
Step 3. Playwright MCP로 기능 테스트 실행 - 정상 케이스 (Happy path) 검증 - 에러/예외 케이스 검증 - 경계값 및 엣지 케이스 검증
...
> **🚫 테스트 없이 다음 단계 진행 금지**: API나 비즈니스 로직을 구현한 후 테스트를 건너뛰고 다음 작업을 진행하는 것은 허용되지 않습니다.
...
- **Task 008: 추가 API 및 인터랙션 구현**
- 부가 API 엔드포인트 구현 (재발급, 캐시 무효화 등)
- 다크모드 및 인쇄 최적화 CSS 적용
- 사용자 경험 개선 및 엣지 케이스 처리

### Phase 5: 최적화 및 배포

- **Task 009: 실제 데이터 연동 및 E2E 테스트** - 우선순위
- 실제 외부 서비스(DB, API) 데이터 연동 및 샘플 데이터 입력
- 환경 변수 설정 및 시크릿 관리 (`NOTION_API_KEY` 등)
- Playwright MCP를 활용한 전체 사용자 플로우 E2E 테스트
- 정상·에러·엣지 케이스 전 구간 검증
- 크로스 브라우저(Chrome·Safari·Edge) 테스트

- **Task 010: 성능 최적화 및 프로덕션 배포**
- Lighthouse 성능 측정 (LCP < 2.5s, TTFB < 400ms)
- 캐싱 전략 검증 및 최적화
- CI/CD 파이프라인 구축
- Vercel 등 프로덕션 환경 배포 및 환경 변수 등록
- 모니터링 및 로깅 시스템 구성
```

### 🧪 Playwright MCP 테스트 전략

API 연동 및 비즈니스 로직 구현 후에는 아래 절차에 따라 Playwright MCP를 사용하여 테스트를 수행합니다.

#### **테스트 대상 (필수)**

| 구현 유형                          | 테스트 필수 여부 | 테스트 시점             |
| ---------------------------------- | ---------------- | ----------------------- |
| API 엔드포인트 구현                | ✅ 필수          | 각 엔드포인트 구현 직후 |
| 비즈니스 로직 (데이터 처리, 계산)  | ✅ 필수          | 로직 구현 직후          |
| 인증/권한 시스템                   | ✅ 필수          | 각 플로우 구현 직후     |
| 폼 제출 및 데이터 검증             | ✅ 필수          | 폼 기능 구현 직후       |
| 외부 서비스 연동 (Notion, 결제 등) | ✅ 필수          | 연동 구현 직후          |
| UI 렌더링 (정적 컴포넌트)          | ⚡ 권장          | Phase 2 완료 시점       |

#### **단계별 테스트 절차**

```
[1단계] 개발 서버 시작 확인
  → mcp__playwright__browser_navigate로 localhost 접속
  → 페이지 정상 로드 확인

[2단계] 정상 케이스 (Happy Path) 테스트
  → 기능이 의도대로 동작하는 시나리오 실행
  → 응답 데이터 및 UI 변경 확인
  → mcp__playwright__browser_snapshot으로 결과 캡처

[3단계] 에러/예외 케이스 테스트
  → 잘못된 입력값, 빈 값, 권한 없는 접근 등 테스트
  → 에러 메시지 표시 및 적절한 처리 확인

[4단계] 경계값 및 엣지 케이스 테스트
  → 최대/최솟값, 특수문자, 대용량 데이터 등

[5단계] 네트워크 요청 검증
  → mcp__playwright__browser_network_requests로 API 호출 확인
  → 요청/응답 데이터 정합성 검증

[6단계] 콘솔 에러 확인
  → mcp__playwright__browser_console_messages로 에러 로그 확인
  → 경고 및 에러 없음 확인
```

#### **테스트 시나리오 작성 템플릿**

각 Task의 작업 파일(`/tasks/XXX-description.md`)에 아래 섹션을 필수 포함:
...
- **Phase 4: 고급 기능 및 추가 기능**
  - 부가 기능 및 고급 사용자 경험
  - 추가 API 엔드포인트 (재발급, 캐시 무효화 등)
  - 다크모드, 인쇄 최적화 등 사용자 경험 개선

- **Phase 5: 최적화 및 배포**
  - 실제 외부 서비스 데이터 연동 및 샘플 데이터 입력
  - 환경 변수 설정 및 프로덕션 배포 (Vercel 등)
  - Playwright MCP E2E 통합 테스트 (전체 플로우, 크로스 브라우저)
  - Lighthouse 성능 측정 및 최적화 (LCP, TTFB 기준 달성)
  - CI/CD 파이프라인 구축 및 모니터링 설정
...
- [ ] Phase 4에서 추가 기능과 사용자 경험 개선이 이루어지는가?
- [ ] Phase 5에서 실제 데이터 연동 테스트, 배포, 성능 측정이 포함되는가?
...
- [ ] 테스트 체크리스트에 정상/에러/엣지 케이스가 모두 포함되었는가?
...
- [ ] 구현 직후 테스트 수행 원칙이 워크플로우에 명시되었는가?
- [ ] 테스트 미통과 시 다음 단계 진행 금지 원칙이 반영되었는가?
- [ ] Playwright MCP 6단계 테스트 절차(정상→에러→엣지→네트워크→콘솔)가 Task에 안내되었는가?
