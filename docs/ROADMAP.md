...
> **버전**: 0.5.0 | **최종 업데이트**: 2026-02-23 | **기반 문서**: PRD v0.1.0
>
> 상태 표기: ✅ 완료 | 🔄 진행 중 | ⬜ 미완료
...
| #   | 지표                  | 목표 기준                               | 현재 상태                                  |
| --- | --------------------- | --------------------------------------- | ------------------------------------------ |
| 1   | 고객 첫 열람 LCP      | **< 2.5s** (ISR + RSC)                  | ✅ 달성 예상 (TTFB dev: ~0.9s, 번들 144KB) |
| 2   | PDF 다운로드 성공률   | 방문자의 **80% 이상** 에러 없이 저장    | ✅ E2E 검증 완료 (5.6)                     |
| 3   | 관리자 발행 소요 시간 | Notion 입력 → URL 공유까지 **2분 이내** | ✅ 어드민 대시보드에서 즉시 링크 복사 가능 |
...
| **관리자 대시보드**  | **고도화**  | `app/admin/invoices/page.tsx`                  | ✅          |
| **링크 복사 버튼**   | **고도화**  | `components/admin/copy-link-button.tsx`        | ✅          |
| **다크모드 토글 UI** | **고도화**  | `components/ui/theme-toggle.tsx`               | ✅          |
...
| **Phase 6** | **6.3**       | **어드민 인증 API (로그인/로그아웃)** | ✅ 필수                  |
| **Phase 6** | **6.5**       | **견적서 목록 페이지 E2E**            | ✅ 필수                  |
| **Phase 6** | **6.6**       | **링크 복사 클립보드 동작 검증**      | ✅ 필수                  |
| **Phase 6** | **6.7**       | **다크모드 토글 및 테마 전환 검증**   | ✅ 필수                  |
| **Phase 6** | **6.8**       | **어드민 전체 플로우 통합 E2E**       | ✅ 필수                  |
...
| 5.3 | Vercel 배포 및 환경 변수 설정              | Vercel 대시보드     | ✅   | `NOTION_*`, `ADMIN_SECRET` 환경 변수 등록 완료                                                                                  |
| 5.4 | **[테스트]** API 엔드포인트 E2E 검증       | —                   | ✅   | `GET /api/invoice/[slug]` 정상·404·만료, `POST /api/admin/…/regenerate` 인증·슬러그 재발급, `POST /api/revalidate` 7케이스 통과 |
| 5.5 | **[테스트]** 뷰어 UI E2E 테스트            | —                   | ✅   | 견적서 조회·렌더링·상태 분기(Draft/Expired/미존재) 6케이스 통과, 반응형 레이아웃 확인                                           |
| 5.6 | **[테스트]** 실제 Notion 데이터로 E2E 검증 | —                   | ✅   | 전체 플로우: 견적서 조회→PDF 다운로드→인쇄 다이얼로그, ISR revalidate=60 확인, 에러 케이스 검증                                 |
| 5.7 | 성능 측정 (간이)                           | —                   | ✅   | TTFB(dev)~0.9s, 번들 144KB(<150KB) ✅, ISR 프로덕션에서 TTFB < 400ms 예상                                                       |

---

## Phase 6 — 관리자 대시보드 + 다크모드 토글 ✅

**목표**: 관리자 전용 견적서 목록 페이지 구현, 링크 복사 기능, 다크모드 토글 UI 추가

### 신규 생성 파일

| 파일 경로                                        | 역할                                           |
| ------------------------------------------------ | ---------------------------------------------- |
| `src/app/admin/layout.tsx`                       | 어드민 공통 레이아웃 + 쿠키 인증 가드          |
| `src/app/admin/page.tsx`                         | `/admin` → `/admin/invoices` 리다이렉트        |
| `src/app/admin/login/page.tsx`                   | 어드민 로그인 폼 페이지                        |
| `src/app/admin/invoices/page.tsx`                | 견적서 목록 서버 컴포넌트 페이지               |
| `src/app/api/admin/auth/login/route.ts`          | POST 로그인 API (HttpOnly 쿠키 발급)           |
| `src/app/api/admin/auth/logout/route.ts`         | POST 로그아웃 API (쿠키 삭제)                  |
| `src/components/admin/admin-header.tsx`          | 어드민 헤더 (로그아웃 + ThemeToggle)           |
| `src/components/admin/invoice-list-table.tsx`    | 견적서 목록 테이블 (shadcn/ui Table)           |
| `src/components/admin/invoice-status-filter.tsx` | 상태 필터 Select (URL 쿼리파라미터)            |
| `src/components/admin/copy-link-button.tsx`      | 링크 복사 버튼 (Clipboard API + sonner 토스트) |
| `src/components/ui/theme-toggle.tsx`             | 다크모드 토글 버튼 (Sun/Moon 아이콘)           |

### 수정 필요 파일

| 파일 경로                         | 수정 내용                                               |
| --------------------------------- | ------------------------------------------------------- |
| `src/types/invoice.ts`            | `InvoiceListItem` 타입 추가 (`slug` 포함, `items` 제외) |
| `src/lib/notion.ts`               | `getAllInvoices()` 함수 추가 (1회 API 호출, N+1 해결)   |
| `src/middleware.ts`               | `/admin/*` 쿠키 검증 로직 + matcher 확장                |
| `src/app/api/revalidate/route.ts` | `revalidatePath('/admin/invoices')` 추가                |

### 작업 상세

| #   | 작업                                              | 파일 경로                                                  | 상태 | 비고                                                              |
| --- | ------------------------------------------------- | ---------------------------------------------------------- | ---- | ----------------------------------------------------------------- |
| 6.1 | `InvoiceListItem` 타입 정의                       | `src/types/invoice.ts`                                     | ✅   | `items` 제외, `slug` 포함 — 목록 조회 전용 경량 타입              |
| 6.2 | `getAllInvoices()` Notion 목록 조회 함수          | `src/lib/notion.ts`                                        | ✅   | `dataSources.query()` 1회 호출, `unstable_cache` 120s             |
| 6.3 | 어드민 인증 API (로그인/로그아웃) + 미들웨어 확장 | `src/app/api/admin/auth/*/route.ts`, `src/middleware.ts`   | ✅   | HttpOnly 쿠키 `admin-session` 발급, `/admin/*` 쿠키 검증          |
| 6.4 | 어드민 레이아웃 + 로그인 페이지                   | `src/app/admin/layout.tsx`, `src/app/admin/login/page.tsx` | ✅   | 미들웨어에서 미인증 시 `/admin/login` 리다이렉트                  |
| 6.5 | 견적서 목록 페이지 + 테이블 컴포넌트              | `src/app/admin/invoices/page.tsx`, `src/components/admin/` | ✅   | 상태 필터(URL 쿼리파라미터+클라이언트 필터링), shadcn/ui Table    |
| 6.6 | 링크 복사 버튼                                    | `src/components/admin/copy-link-button.tsx`                | ✅   | Clipboard API, 2초간 CheckIcon → Link2Icon, sonner 토스트         |
| 6.7 | 다크모드 토글 버튼                                | `src/components/ui/theme-toggle.tsx`                       | ✅   | `next-themes` `useTheme()`, 어드민 헤더에 적용                    |
| 6.8 | **[테스트]** Phase 6 통합 E2E                     | `scripts/test-admin-e2e.ts`                                | ✅   | 9/9 테스트 통과: 로그인/401/인증가드/목록/로그아웃/필터/복사/토글 |
...
| OQ-6 | 어드민 인증 강화 — `ADMIN_SECRET` 쿠키 방식 보안 | Phase 6: HttpOnly 쿠키 기반 간이 세션 → Post-MVP: Clerk/NextAuth 또는 HMAC 서명 토큰 전환 | ⬜ 결정 필요 |
...
| Clerk/NextAuth 관리자 인증   | HttpOnly 쿠키 `ADMIN_SECRET` 방식 대체      | Medium   | 관리자 2인 이상 사용 시 |
...
| 캐싱 전략     | ISR `revalidate: 60` (목록: 120s)     | Notion API rate limit(3req/s) 대응, 반복 요청 최소화                |
| PDF 생성 방식 | `window.print()` + CSS `@media print` | `@react-pdf/renderer` 대비 번들 크기(-300KB), 한글 폰트 설정 불필요 |
| 어드민 인증   | HttpOnly 쿠키 기반 간이 세션          | MVP 빠른 구현 → Post-MVP에서 Clerk/NextAuth 전환 예정               |
| 슬러그 방식   | UUID v4 (약 5.3×10³⁶ 조합)            | 브루트포스 불가, 유출 시 재발급 API로 대응                          |
| 다크모드      | `next-themes` + CSS 변수              | 이미 설치 완료, `ThemeToggle` 컴포넌트만 추가하면 동작              |
| 링크 복사     | Clipboard API + sonner 토스트         | `sonner` 이미 설치, `window.location.origin` 폴백 포함              |

---

_이 문서는 `docs/PRD.md` v0.1.0을 기반으로 작성되었으며, Phase 진행 중 상태를 지속 업데이트합니다._
_MVP 로드맵 원본: `docs/roadmaps/ROADMAP_v1.md`_
